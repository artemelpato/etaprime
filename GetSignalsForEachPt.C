...
void GetSignalsForEachPt(const double pt_min,
                         const double pt_max,
                         const char *infile_name,
                         const char *outfile_name)
{
    const double substraction_scaling = 0.97;
        ...
        for (int vertex = 0; vertex < n_vertexes; ++vertex)
        {
            for (int pid = 0; pid < n_pids; ++pid)
            {
                for (int cut = 0; cut < n_cuts; ++cut)
                {
                    ...
                    TH1D *peak_integral_hist = (TH1D*) fg_hist->ProjectionX(peak_hist_name);
                    peak_integral_hist->Reset();
                    ...
                    for (int pt_bin = pt_min_bin; pt_bin <= pt_max_bin; ++pt_bin)
                    {
                        ...
                        char peak_hist_temp_name[100];

                        sprintf(fg_hist_proj_name,   "fg_%s_proj",   dirname);
                        sprintf(bg_hist_proj_name,   "bg_%s_proj",   dirname);
                        sprintf(peak_hist_temp_name, "peak_%s_proj", dirname);

                        TH1D *fg_hist_proj = fg_hist->ProjectionX(fg_hist_proj_name, pt_bin, pt_bin);
                        TH1D *bg_hist_proj = bg_hist->ProjectionX(bg_hist_proj_name, pt_bin, pt_bin);

                        TH1D *peak_hist = (TH1D*) fg_hist_proj->Clone(peak_hist_temp_name);
                        ...
                        peak_integral_hist->Add(peak_hist);
                    }
